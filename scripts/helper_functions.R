...
# Generator <- function(sce, phenoData, Num.mixtures = 1000, pool.size = 100, min.percentage = 1, max.percentage = 99, seed = 24){ CT =
# unique(phenoData$cellType) ?stopifnot(length(CT) >= 2) set.seed(seed) require(dplyr) require(gtools) cell.distribution =
# data.frame(table(phenoData$cellType),stringsAsFactors = FALSE) colnames(cell.distribution) = c('CT','max.n') Tissues = list() Proportions = list() for(y in
# 1:Num.mixtures){ #Only allow feasible mixtures based on cell distribution while(!exists('P')){ num.CT.mixture = sample(x = 2:length(CT),1) selected.CT =
# sample(CT, num.CT.mixture, replace = FALSE) P = runif(num.CT.mixture, min.percentage, max.percentage) P = round(P/sum(P), digits = log10(pool.size)) #sum to 1
# P = data.frame(CT = selected.CT, expected = P, stringsAsFactors = FALSE) missing.CT = CT[!CT %in% selected.CT] missing.CT = data.frame(CT = missing.CT,
# expected = rep(0, length(missing.CT)), stringsAsFactors = FALSE) P = rbind.data.frame(P, missing.CT) potential.mix = merge(P, cell.distribution)
# potential.mix$size = potential.mix$expected * pool.size if( !all(potential.mix$max.n >= potential.mix$size) | sum(P$expected) != 1){ rm(list='P') } }

# # Using info in P to build T simultaneously chosen_cells <- sapply(which(P$expected != 0), function(x){ n.cells = P$expected[x] * pool.size chosen =
# sample(phenoData$cellID[phenoData$cellType == P$CT[x]], n.cells) chosen }) %>% unlist() T <- Matrix::rowSums(sce[,colnames(sce) %in% chosen_cells]) %>%
# as.data.frame() colnames(T) = paste('mix',y,sep='') P = P[,c('CT','expected')] P$mix = paste('mix',y,sep='') Tissues[[y]] <- T Proportions[[y]] <- P
# rm(list=c('T','P','chosen_cells','missing.CT')) } P = do.call(rbind.data.frame, Proportions) T = do.call(cbind.data.frame, Tissues) P = data.table::dcast(P, CT
# ~ mix, value.var = 'expected', fun.aggregate = sum) %>% data.frame(.,row.names = 1) P = P[,gtools::mixedsort(colnames(P))] return(list(T = T, P = P)) }
...
marker.fc <- function(fit2, log2.threshold = 1, output_name = "markers") {
    ...
    markers <- cbind.data.frame(rownames(topTable_RESULTS), t(apply(topTable_RESULTS, 1, function(x) {
        temp <- max(x)
...
Scaling <- function(matrix, option, phenoDataC = NULL) {
    ...
    ########## Remove rows & columns full of zeroes ########## Error: NMF::nmf - Input matrix x contains at least one null or NA-filled row. matrix =
    ########## matrix[rowSums(matrix)!=0,] OLS with error if all elements within a row are equal (e.g. all 0, or all a common value after log/sqrt/vst
    ########## transformation)
    ...
    if (option == "column") {
        ...
        colnames(matrix) <- matrix_colnames

    } else if (option == "TMM") {
        # CPM counts coming from TMM-normalized library sizes; https://support.bioconductor.org/p/114798/

        if (!is.null(phenoDataC)) {
            # use CT info for scRNA-seq
        ...
        ## following line needed to solve 'Wrong R type for mapped matrix' https://github.com/ChristophH/sctransform/issues/24
        ...
        # It is not compatible with log transformed datasets.
...
Deconvolution <- function(T, C, method, phenoDataC, P = NULL, elem = NULL, STRING = NULL, marker_distrib, refProfiles.var) {
    ...
    bulk_methods <- c("DeconRNASeq", "OLS", "nnls", "FARDEEP", "RLR", "DCQ", "elastic_net", "lasso", "ridge", "EPIC", "DSA", "ssKL", "ssFrobenius", "dtangle")
    ...
    ################################### without QN. By default, CIBERSORT performed QN (only) on the mixture.
    if (method == "CIBERSORT") {
        ...
        # nonnegative quadratic programming; lsei function (default: type=1, meaning lsei from quadprog) datasets and reference matrix: signatures, need to be
        # non-negative.
        ...
        RESULTS <- t(DeconRNASeq::DeconRNASeq(datasets = as.data.frame(T), signatures = as.data.frame(C), proportions = NULL, checksig = FALSE, known.prop = FALSE,
            use.scale = FALSE, fig = FALSE)$out.all)
        ...
        RESULTS <- apply(T, 2, function(z) coef(glmnet::glmnet(x = as.matrix(C), y = z, alpha = 0.2, standardize = TRUE, lambda = glmnet::cv.glmnet(as.matrix(C), z)$lambda.1se))[1:ncol(C) +
            1, ])
        RESULTS <- apply(RESULTS, 2, function(x) ifelse(x < 0, 0, x))  #explicit non-negativity constraint
        RESULTS <- apply(RESULTS, 2, function(x) x/sum(x))  #explicit STO constraint
        ...
        require(glmnet)
        RESULTS <- apply(T, 2, function(z) coef(glmnet::glmnet(x = as.matrix(C), y = z, alpha = 0, standardize = TRUE, lambda = glmnet::cv.glmnet(as.matrix(C), z)$lambda.1se))[1:ncol(C) +
            1, ])
        RESULTS <- apply(RESULTS, 2, function(x) ifelse(x < 0, 0, x))  #explicit non-negativity constraint
        RESULTS <- apply(RESULTS, 2, function(x) x/sum(x))  #explicit STO constraint
        ...
        require(glmnet)
        RESULTS <- apply(T, 2, function(z) coef(glmnet::glmnet(x = as.matrix(C), y = z, alpha = 1, standardize = TRUE, lambda = glmnet::cv.glmnet(as.matrix(C), z)$lambda.1se))[1:ncol(C) +
            1, ])
        RESULTS <- apply(RESULTS, 2, function(x) ifelse(x < 0, 0, x))  #explicit non-negativity constraint
        RESULTS <- apply(RESULTS, 2, function(x) x/sum(x))  #explicit STO constraint
        ...
        require(MuSiC)
        RESULTS <- t(MuSiC::music_prop(bulk.eset = T.eset, sc.eset = C.eset, clusters = "cellType", markers = NULL, normalize = FALSE, samples = "SubjectName", verbose = F)$Est.prop.weighted)
        ...
        if (!dir.exists(path)) {
            # to avoid repeating marker_selection step when removing cell types; Sig.RData automatically created
            ...
            # re-load signature and remove CT column + its correspondent markers

            load(paste(path, "Sig.RData", sep = "/"))
            Signature <- Sig

            if (!is.null(elem)) {
                # to be able to deal with full C and with removed CT

                Signature <- Signature[, !colnames(Signature) %in% elem]
                CT_to_read <- dir(path) %>%
                  grep(paste(elem, ".*RData", sep = ""), ., value = TRUE)
        ...
        # By default, BisqueRNA uses all genes for decomposition. However, you may supply a list of genes (such as marker genes) to be used with the markers
        # parameter
        ...
        require(SCDC)
        RESULTS <- t(SCDC::SCDC_prop(bulk.eset = T.eset, sc.eset = C.eset, ct.varname = "cellType", sample = "SubjectName", ct.sub = unique(as.character(phenoDataC$cellType)),
    ...
    T <- as.data.frame(T)
    T$CT <- rownames(T)
    T <- data.table::melt(T, id.vars = "CT")
    ...
    if (!is.null(P)) {
        ...
        # str<- str(P) write.csv(str, 'C:\\Users\\elhammajd\\Documents\\pipeline2\\stroriginal.csv') saveRDS(P,
        # 'C:\\Users\\elhammajd\\Documents\\pipeline2\\Pfinal.rds')
